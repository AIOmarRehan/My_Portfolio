...
export default function FullstackProjectDetailsPage() {
  ...
  const [aiProjects, setAiProjects] = useState<CarouselItem[]>([])
  ...
  const otherFullstack = useMemo(
    () => allFullstack.filter((p) => p.id !== parseInt(projectId)),
    [allFullstack, projectId]
  )
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center">
        <div className="neo-card neo-card-alt text-center px-10 py-12 -rotate-1">
          ...
          <Link href="/#fullstack-projects" className="neo-btn neo-btn-cyan">Back to Full-Stack Projects</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <main className="min-h-screen pb-12 overflow-x-clip">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-3">
          ...
          {(project.github_url || project.live_project_link) && (
            <div className="flex flex-wrap gap-4 mb-8">
              ...
              {project.live_project_link && (
                <a href={project.live_project_link} target="_blank" rel="noopener noreferrer" className="neo-btn neo-btn-lime">
          ...
          {project.tags && project.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-6" style={{ borderTop: 'var(--neo-bw) solid var(--neo-border)' }}>
              {project.tags.map((tag, idx) => (
                <TagBadge key={idx} tag={tag} variant="gray" />
              ))}
            </div>
          )}
        </div>
        ...
        {project.demo_video && (
          <div className="mb-16">
            <h2 className="text-2xl font-extrabold mb-6 inline-block bg-neo-cyan border-neo border-neo-border px-3 py-1.5 shadow-neo-sm -rotate-1">Demo Video</h2>
        ...
        <ProjectCarousel title="More Full-Stack Projects" items={otherFullstack} hrefBase="/fullstack-projects" accent="cyan" />
